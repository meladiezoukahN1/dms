...
import { ArchiveHandoverValidator } from "@/server/modules/correspondence/archive-handover/validator";
...
async function getHandler(req: NextRequest, fileId: string): Promise<NextResponse> {
  ...
  const validatedFileId = ArchiveHandoverValidator.validateId(fileId, "معرّف الملف مطلوب");

  const fileAccess = await ArchiveHandoverService.getDownloadFileAccess(
    validatedFileId,
    currentUser.status
  );
