...
import type { UpdateArchiveHandoverOutput } from "@/server/modules/correspondence/archive-handover/types";
...
async function patchHandler(req: NextRequest): Promise<NextResponse> {
  ...
  const result = await ArchiveHandoverService.updateMetadata(
    id,
    body,
    currentUser.id,
    currentUser.status,
  ...
  return NextResponse.json(
    {
      success: true,
      data: result,
    } satisfies ApiResponse<UpdateArchiveHandoverOutput>,
    { status: 200 }
  );
}
