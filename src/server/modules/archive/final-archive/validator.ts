...
export class FinalArchiveValidator {
  static validateListQuery(raw: unknown): FinalArchiveListQuery {
  ...
  static validateUpdateInput(raw: unknown): UpdateFinalArchiveInput {
    const schema = z.object({
    ...
    try {
      ...
      throw new AppError("INVALID_INPUT", 400, "بيانات التحديث غير صحيحة");
    }
  }

  static validateArchiveInput(raw: unknown): ArchiveCorrespondenceInput {
    const schema = z.object({
    ...
    try {
      ...
      throw new AppError("INVALID_INPUT", 400, "بيانات الأرشفة غير صحيحة");
    }
  }
